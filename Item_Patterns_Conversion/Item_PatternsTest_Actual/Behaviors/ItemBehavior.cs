...
namespace Item_PatternsTest_Actual.Behaviors
{
    ...
    //This also implements the "Flyweight" design pattern, since _name, _id, _description, and _weight are all intrisict to their respective BASE items
    //and don't change (Unless a tool item is decorated as an EnchatedTool, in which case it has an override implementation).
    ...
    public class ItemBehavior : IItemBehavior
    {
        ...
        protected int _id; // All items have an associated ID (To be used with a GetItemByID() method to be ported over from the VR game later) 
        ...
        public virtual int ID() { return _id; }
        public virtual float Weight() { return _weight; }
        public virtual string BaseDescription()
        { //Provide the base description/flavor text for the item
            string itemDescription = "\nDescription: " + _description;
            return itemDescription;
        }


        public virtual void ChangeName(string newName) { _name = newName; } //Test function, not meant for the final product. Used for testing the Flyweight design pattern
    }
}
